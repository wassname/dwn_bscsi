...
for name, row in tqdm(files.iterrows(), total=len(files), desc='files'):
    ...
    if not dest.is_file():
        ...
        with TqdmUpTo(
            unit="B", unit_scale=True, unit_divisor=1024, mininterval=1, desc=str(row.rel_path)
        ) as t:
            urllib.request.urlretrieve(row.url, dest_temp, reporthook=t.update_to)
            if check_size(row, dest_temp, rm=True):
                shutil.move(dest_temp, dest)

    else:
        check_size(row, dest, rm=True)
